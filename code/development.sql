SELECT visit_id, request_id, url, headers, top_level_url, resource_type,
    time_stamp, post_body, post_body_raw from http_requests where 22591938272157 = visit_id;

SELECT visit_id, request_id, url, headers, response_status, time_stamp, content_hash
    from http_responses where 22591938272157 = visit_id;

SELECT visit_id, old_request_id, old_request_url, new_request_url, response_status,
            headers, time_stamp from http_redirects where 22591938272157 = visit_id;

SELECT visit_id, request_id, call_stack from callstacks where 22591938272157 = visit_id;

SELECT visit_id, script_url, script_line, script_loc_eval, top_level_url, document_url, symbol, call_stack, operation,
            arguments, attributes, value, time_stamp from javascript where 22591938272157 = visit_id;
